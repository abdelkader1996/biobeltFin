...
export class HomePage {
  ...
  constructor(
    private ucp3service: Upcv3serviceService,
  ...
  async ionViewWillEnter() {
    this.mode_test = await this.storage.get("mode_test");
      ...
      this.storage.get("isInterventionNotSaved").then((res) => {
        if (res == true) {
          async () => {
            ...
            loading.present();
            var arr = await Promise.all([this.getJson(), this.getToken()]);
  ...
  createIntervention() {
    this.ucp3service.createIntervention(this.json, this.token).subscribe(
      (res) => {
    ...
    return new Promise<void>(async (resolve, reject) => {
      this.alertController
        .create({
          ...
          message:
            "Une intervention est en cours, souhaitez-vous restaurer les paramètres ?",
          buttons: [
            {
              text: "Non",
              handler: () => {
  ...
  async testMode($event) {
    ...
    await this.storage.set("ssid_upc", wifi);
    ...
    return new Promise<void>(async (resolve, reject) => {
      this.alertController
        .create({
          ...
          subHeader: "Intervention en cours",
          message:
            "L'intervention n'a pas pu être enregistrée, souhaitez-vous réessayer (vérifiez votre connexion internet) ?",
          buttons: [
            {
              text: "Non",
              role: "cancel",
